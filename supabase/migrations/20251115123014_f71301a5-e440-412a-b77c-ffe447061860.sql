-- Add availability columns to provider_profiles
ALTER TABLE public.provider_profiles
ADD COLUMN available_now boolean DEFAULT false,
ADD COLUMN available_now_started_at timestamp with time zone,
ADD COLUMN available_now_location jsonb,
ADD COLUMN accept_invitations boolean DEFAULT true;
...
CREATE INDEX idx_provider_profiles_available_now ON public.provider_profiles(available_now) WHERE available_now = true;
CREATE INDEX idx_provider_profiles_accept_invitations ON public.provider_profiles(accept_invitations) WHERE accept_invitations = true;
CREATE INDEX idx_client_invitations_status ON public.client_invitations(status, expires_at);
CREATE INDEX idx_client_invitations_client_id ON public.client_invitations(client_id);
CREATE INDEX idx_invitation_responses_invitation_id ON public.invitation_responses(invitation_id);
CREATE INDEX idx_invitation_responses_provider_id ON public.invitation_responses(provider_id);
